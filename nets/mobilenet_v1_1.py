...
def mobilenet_v1_base(inputs,
                      final_endpoint='Conv2d_1_13_pointwise',
                      min_depth=8,
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding):
      ...
      for i, conv_def in enumerate(conv_defs):
        if i == 1:
          if i <= merged_end_layer:
            end_point_base = 'Conv2d_%d' % i
          else:
            end_point_base = 'Conv2d_1_%d' % i
        elif i == 2:
          if i <= merged_end_layer:
            end_point_base = 'Conv2d_%d' % i
          else:
        ...
        if isinstance(conv_def, Conv):
          ...
          if i == 1:
            if i <= merged_end_layer:
              end_point = end_point_base + '_depthwise_1'
            else:
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs], reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      net, end_points = mobilenet_v1_base(inputs, scope=scope,
                                          ...
                                          conv_defs=conv_defs,
                                          merged_end_layer=merged_end_layer)
      ...
      with tf.variable_scope('Logits'):
        ...
        try:
          ...
          logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=None,
                              normalizer_fn=None, scope='Conv2d_1_1c_1x1',reuse=tf.AUTO_REUSE)
        if spatial_squeeze:
